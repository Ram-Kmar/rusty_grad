...
impl<T: TensorFloat> Backend<T> for CudaBackend {
    ...
    fn zeros(_shape: &[usize], _device: Device) -> Result<TensorData<T>> { todo!() }
    fn random_uniform(_shape: &[usize], _device: Device, _min: f64, _max: f64) -> Result<TensorData<T>> { todo!() }
    fn from_cpu_data(_data: &[T], _shape: &[usize], _device: Device) -> Result<TensorData<T>> { todo!() }
    fn ones(_shape: &[usize], _device: Device) -> Result<TensorData<T>> { todo!() }
    fn add(_a: &TensorData<T>, _b: &TensorData<T>) -> Result<TensorData<T>> { todo!() }
    fn sub(_a: &TensorData<T>, _b: &TensorData<T>) -> Result<TensorData<T>> { todo!() }
    fn mul(_a: &TensorData<T>, _b: &TensorData<T>) -> Result<TensorData<T>> { todo!() }
    fn div(_a: &TensorData<T>, _b: &TensorData<T>) -> Result<TensorData<T>> { todo!() }
    fn matmul(_a: &TensorData<T>, _b: &TensorData<T>) -> Result<TensorData<T>> { todo!() }
    fn relu(_input: &TensorData<T>) -> Result<TensorData<T>> { todo!() }
    fn exp(_input: &TensorData<T>) -> Result<TensorData<T>> { todo!() }
    fn log(_input: &TensorData<T>) -> Result<TensorData<T>> { todo!() }
    fn transpose(_input: &TensorData<T>) -> Result<TensorData<T>> { todo!() }
}
